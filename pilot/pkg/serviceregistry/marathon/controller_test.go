...
var (
	...
	productpage = []*marathon.Application{
		{
			//Node:           "istio",
			//Address:        "172.19.0.5",
			ID:             "/app",
			PortDefinitions: &[]marathon.PortDefinition{{nil,"tcp",nil,nil}},
			Ports:    []int{9079},
	...
	reviews = []*marathon.Task{
		{
			...
			Version:		"2018_0126",
			Ports:          []int{9090},
			IPAddresses:    []*marathon.IPAddress{{nil,"tcp"},},
			//ServiceName:    "reviews",
			//ServiceTags:    []string{"version|v1"},
...
func newServer() *mockServer {
	m := mockServer{
		Productpage: make([]*marathon.Application, len(productpage)),
		Reviews:     make([]*marathon.Task, len(reviews)),
